        ...
        def eval_highs():
            ...
            if is_trading == 1 and valid_rn >= 2:
                ...
                if prior_trading_highs:
                    ...
                    # 1. 覆盖期新高 (严谨求真口径: 必须具备 >= 500 有效交易日深度)
                    if listing_tdays > 500 and len(prior_trading_highs) >= 500 and high_t0 > max_all_prior:
                        is_ath = 1

                    # 2. 500日新高 (严谨求真口径: 必须具备 >= 500 有效交易日深度)
                    if listing_tdays > 500 and len(prior_trading_highs) >= 500 and high_t0 > max(prior_trading_highs[-500:]):
                        is_h500 = 1

                    # 3. 250日新高 (严谨求真口径: 必须具备 >= 250 有效交易日深度)
                    if listing_tdays > 250 and len(prior_trading_highs) >= 250 and high_t0 > max(prior_trading_highs[-250:]):
                        is_h250 = 1
