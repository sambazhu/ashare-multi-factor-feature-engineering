...
check("最新日科创板有效成交额覆盖率 = 100%",
      len(star_valid_amo) == len(latest_star),
...
for i, r in enumerate(rows):
    ...
    if (int(ha) + int(h5) + int(h2)) > 1: viol.append((r["SECUCODE"], r["T0"], "三种新高同时命中"))
    ...
    # LABEL 一致性 (交接文档定义: 必须满排列+全升+有效交易日)
    if r["LABEL"] == "1":
...
check("不变式: 三种新高强制互斥 (需求§5)", not any("三种新高" in v[2] for v in viol))
